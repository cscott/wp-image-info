...
var Db = module.exports = function(filename, options) {
    ...
    var _db = new sqlite3.Database(filename, mode, deferred.callback);
    var db = {
        get: Promise.promisify(_db.get, _db),
        each: Promise.promisify(_db.each, _db),
        run: Promise.promisify(_db.run, _db),
        close: Promise.promisify(_db.close, _db)
    };
...
Db.prototype.count = function() {
    return this.db.then(function(db) {
        return db.get("SELECT count() AS count FROM kv_table;");
    }).then(function(row) {
        return row.count;
    });
};
...
Db.prototype.get = function(key, nojson) {
    return this.db.then(function(db) {
        return db.get("SELECT val FROM kv_table WHERE key = ?;",
                      '' + key);
    }).then(function(row) {
        var val = (row || {}).val;
...
Db.prototype.forEach = function(f, nojson) {
    ...
    return this.db.then(function(db) {
        return db.each("SELECT * FROM kv_table;", each);
    });
};
