...
exports.main = function() {
    return createResultDb(PAGEDB).then(function(pagedb) {
        ...
        return db.prepare("SELECT DISTINCT pagename,pageid FROM usage ORDER BY pagename,pageid ASC;").
            then(function(stmt) {
                var handleRow = function(row) {
                    if (!row) return; // done
                    // xxx we want more parallelism here
                    return processPage(pagedb, row.pagename, row.pageid).
                        then(function() {
