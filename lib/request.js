...
var LengthStream = function() {
    stream.Writable.call(this, { decodeStrings: true });
    this.length = 0;
};
util.inherits(LengthStream, stream.Writable);
LengthStream.prototype._write = function(chunk, encoding, callback) {
    this.length += chunk.length;
    callback();
};
LengthStream.prototype.toString = function() {
    return '' + this.length;
};
...
var request = module.exports = function(requestOptions) {
    return new Promise(function(resolve, reject) {
        ...
        var ss = requestOptions.lengthOnly ?
            new LengthStream() : new StringStream();
        var resp = null;
        r.pipe(ss, { end: false });
